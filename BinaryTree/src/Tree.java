public class Tree {
    ...
    class Node {
        int value;
        Node left;
        Node right;//узлы левые и правые потомки
        Color color;
    }
    ...
    private void insert(Node node, int value) { // два параметра: узел, в которые мы пришли и з значение, которое мы хотим вставить
        if(node.value != value) {
            if(node.value < value){
                if(node.right  == null) {
                    node.right = new Node();
